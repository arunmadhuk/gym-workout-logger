...
class WorkoutSessionsListAPIView(ListAPIView):
    ...
    def get(self, request):
        ...
        paginated_workout_sessions = paginator.paginate_queryset(workout_qs, request)
        serializer = self.serializer_class(paginated_workout_sessions, many=True)
...
class WorkoutSessionAPIView(APIView):
    ...
    def put(self, request, session_id=None):
        ...
        print(f"put request received for workout session - {session_id} , {request.data}")
        ...
        serializer = self.serializer_class(workout_session_qs, data=request.data, partial=False)
        print(f"serializer is valid - {serializer.is_valid()} , errors - {serializer.errors}")
        if serializer.is_valid():
            workout_session = serializer.save()
            return Response({
    ...
    def patch(self, request):
        session_id = request.query_params.get('session_id')
        print(f"patch request received for workout session - {session_id} , {request.data}")
        if not session_id:
            return Response(
                {'error': 'Session ID is required for updating workout session'}, 
                status=status.HTTP_400_BAD_REQUEST
        ...
        serializer = self.serializer_class(workout_session_qs, data=request.data, partial=True)
        print(f"Data to update - {serializer.initial_data} , existing workout session data - {WorkoutSessionSerializer(workout_session_qs).data}")
        print(f"serializer is valid - {serializer.is_valid()} , errors - {serializer.errors}")
        if serializer.is_valid():
            # workout_session = serializer.save()
            return Response({
